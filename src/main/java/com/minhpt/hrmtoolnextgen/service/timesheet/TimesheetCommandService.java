...
public class TimesheetCommandService {
	...
	public TimesheetDto approvalTimesheet(ApprovalTimesheetRequest form) {
		...
		// Authorization check: only MANAGER, HR, or ADMIN can approve/reject timesheets
		UserPrincipalDto currentUser = getCurrentUser();
		boolean isAuthorized = currentUser.getAuthorities().stream()
				.anyMatch(auth -> auth.getAuthority().equals(EUserRole.PROJECT_MANAGER.getAuthority())
						|| auth.getAuthority().equals(EUserRole.HR.getAuthority())
						|| auth.getAuthority().equals(EUserRole.ADMIN.getAuthority()));
		if (!isAuthorized) {
		...
		UserEntity owner = savedEntity.getUserEntity();
		if (owner != null) {
			String ownerIdStr = String.valueOf(owner.getId());
			String ownerEmail = owner.getEmail();
			String ownerName = owner.getUserInfo() != null ? owner.getUserInfo().getFirstName() : "User";
			String status = savedEntity.getStatus().name();

			TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
				@Override
				public void afterCommit() {
					sseService.sendEvent(ownerIdStr, "timesheet.approval",
							Map.of("timesheetId", savedEntity.getId(), "status", status));
					emailService.sendApprovalNotificationEmail(ownerEmail, ownerName, "Timesheet", status);
				}
			});
		}
	...
	private double calculateMaxAllowedHoursForDay(Long userId, LocalDate workingDay) {
		...
		for (DayOffEntity dayOff : dayOffRepository.findAll(spec)) {
			...
			Map<LocalDate, Double> remainingByDate = workHoursCalculatorService.calculateRemainingHours(dayOff.getStartTime(), dayOff.getEndTime());
